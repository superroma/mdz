...
/**
 * Sets up a page by ensuring servers are running and navigating to the specified path
 */
...
/**
 * Waits for the page to load by checking for the title input
 */
...
/**
 * Navigates to a page by clicking on it in the sidebar
 */
...
/**
 * Verifies that navigation occurred
 */
...
/**
 * Fills the title field and submits it
 */
export async function fillTitleAndSubmit(
  page: Page, 
  title: string, 
  submitKey: 'Enter' | 'blur'
): Promise<void> {
...
/**
 * Common button name to test ID mapping
 */
...
/**
 * Clicks a button by name, using test ID if available
 */
export async function clickButton(page: Page, buttonName: string): Promise<void> {
  const testId = BUTTON_MAP[buttonName];
  if (testId) {
